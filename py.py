...
SETTINGS = """
{
    "agent_download_url": "https://github.com/wildfoundry/dataplicity-agent/releases/download/v0.4.32/dataplicity",
    "agent_version": "v0.4.32",
...
def log(text, *args, **kwargs):
    ...
    write_lines = (
        "\n".join("{}: {}".format(caller.ljust(16), line) for line in lines) + "\n"
    )

    try:
        with open_append_binary(LOG_PATH) as log_file:
            log_file.write(write_lines.encode("utf-8", errors="replace"))
    except IOError:
...
""".format(
    ...
    if not apt_update():
        log(
            "Package database update failed, continuing with existing package database..."
        )
...
def show_step(n, msg):
    ...
    global current_step
    current_step = n
    step_msg = "[[ Step {n} of {max} ]] {msg}".format(n=n, max=MAX_STEPS, msg=msg)
    log("-" * 70)
    user(step_msg)
...
def get_agent_dir():
    ...
    agent_dir = os.path.abspath(
        os.path.join(AGENT_BASE_PATH, settings["agent_version"])
    )
    return agent_dir
...
def get_executable_path():
    ...
    executable_path = os.path.abspath(os.path.join(AGENT_BASE_PATH, "dataplicity"))
...
def download(url, path, urlopen=None):
    ...
    try:
        try:
            url_file = (urlopen or urllib.request.urlopen)(url)
        ...
        try:
            ...
            with open_write_binary(path) as write_file:
                ...
                for chunk in iter(read_chunk, b""):
                    ...
                    completed_percentage = 100.0 * (bytes_read / file_size)
                    sys.stdout.write("\r{:.0f}% ".format(completed_percentage))
                    sys.stdout.flush()
            sys.stdout.write("\r")
            sys.stdout.flush()
        except IOError as e:
...
def run_system(*command):
    ...
    try:
        output = subprocess_check_output(command, stderr=subprocess.STDOUT)
        returncode = 0
    except OSError as e:
...
def configure(getpwnam=None):
    ...
    dataplicity_env = ",".join('{}="{}"'.format(k, v) for k, v in sorted(env.items()))
...
def register_device(urlopen=None):
    ...
    register_error_msg = "The installer was unable to contact the Dataplicity server"

    device_name = platform.node() or "New Device"
    cpu_info = get_cpu_info()
    if cpu_info.get("serial"):
        device_name = "{serial}-{device_name}".format(
            serial=cpu_info.get("serial"), device_name=device_name
        )
    log("device name is {}", device_name)
    register_data = {"name": device_name, "serial": serial, "token": settings["token"]}
    post_data = urllib.parse.urlencode(register_data)
    register_url = settings["register_url"]
    log("posting to {}", register_url)

    try:
        url_file = urlopen(register_url, post_data)
    ...
    if not settings["dry_run"]:
        try:
            ...
            if not isdir(dataplicity_home):
                ...
                chown(
                    dataplicity_home, dataplicity_user.pw_uid, dataplicity_user.pw_gid
                )
...
def update_sudoers():
    ...
    try:
        ...
        user("  The installer was unable " "to update sudoers automatically.")
...
def check_supervisor(isdir=None):
    ...
    if not (isdir or os.path.isdir)(SUPERVISOR_CONF_DIR_PATH):
        ...
        raise AbortInstall("Unable to install Supervisor")
...
def congratulations(device_url):
    ...
    if device_url is not None:
        ...
        user(" * {device_url}", device_url=device_url)
    user("")
...
def send_report(report, urlopen=None):
    ...
    urlopen = urlopen or urllib.request.urlopen
    report_post_body = urllib.parse.urlencode(report)
